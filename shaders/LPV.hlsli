...
{
    if (l == 0)
        return 0.282094792; // 1/(2sqrt(PI);
    if (l == 1 && m == -1)
        return -0.4886025119 * xyz.y;
    if (l == 1 && m == 0)
        return 0.4886025119 * xyz.z;
    if (l == 1 && m == 1)
        return -0.4886025119 * xyz.x;
}
...
initLPV_VSOUT VS_initializeLPV(uint vertexID
                               : SV_VertexID)
{
    ...
    if (LPVSpacePos.x < 0.0f || LPVSpacePos.x >= 1.0f)
        outside = true;
    if (LPVSpacePos.y < 0.0f || LPVSpacePos.y >= 1.0f)
        outside = true;
    if (LPVSpacePos.z < 0.0f || LPVSpacePos.z >= 1.0f)
        outside = true;
    ...
    if (outside)
        output.pos.x = LPV3DWidth * 2.0f;

    return output;
}

initLPV_VSOUT VS_initializeLPV_Bilnear(uint vertexIDin
                                       : SV_VertexID)
{
    initLPV_VSOUT output;
    bool outside = false;
...
[maxvertexcount(1)] void GS_initializeLPV(point initLPV_VSOUT In[1], inout PointStream<initLPV_GSOUT> pointStream)
{
    initLPV_GSOUT Out;
    ...
    [maxvertexcount(1)] void GS_initializeLPV_Bilinear(point initLPV_VSOUT In[1], inout PointStream<initLPV_GSOUT> pointStream)
{
    initLPV_GSOUT Out;
    Out.normal = In[0].normal;
    float2 pos;
