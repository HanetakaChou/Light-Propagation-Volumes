...
[numthreads(X_BLOCK_SIZE, Y_BLOCK_SIZE, Z_BLOCK_SIZE)] void PropagateLPV(uint3 threadgroupId
                                                                         : SV_GroupThreadID,
                                                                           uint threadId
                                                                         : SV_GroupIndex,
                                                                           uint3 groupId
                                                                         ...
                                                                         : SV_DispatchThreadID)
{
    ...
    [numthreads(X_BLOCK_SIZE, Y_BLOCK_SIZE, Z_BLOCK_SIZE)] void PropagateLPV_Simple(uint3 threadgroupId
                                                                                    : SV_GroupThreadID,
                                                                                      uint threadId
                                                                                    : SV_GroupIndex,
                                                                                      uint3 groupId
...
VS_PROP_OUTPUT PropagateLPV_VS(float3 Pos
                               : POSITION, float3 Tex
                               : TEXCOORD)
{
    VS_PROP_OUTPUT f;
...
[maxvertexcount(3)] void PropagateLPV_GS(triangle VS_PROP_OUTPUT In[3], inout TriangleStream<GS_PROP_OUTPUT> triStream)
{
    for (int v = 0; v < 3; v++)
    {
        GS_PROP_OUTPUT Out;
