...
{
    if (pos.x >= coarserLevelWidth || pos.x < 1)
        return float4(0, 0, 0, 0);
    if (pos.y >= coarserLevelHeight || pos.y < 1)
        return float4(0, 0, 0, 0);
    if (pos.z >= coarserLevelDepth || pos.z < 1)
        return float4(0, 0, 0, 0);
...
[numthreads(X_BLOCK_SIZE, Y_BLOCK_SIZE, Z_BLOCK_SIZE)] void DownsampleMax(uint threadId
                                                                          : SV_GroupIndex,
                                                                            uint3 groupId
                                                                          : SV_GroupID,
...
{
    bool outside = false;
    if (globalThreadId.x >= coarserLevelWidth || globalThreadId.y >= coarserLevelHeight || globalThreadId.z >= coarserLevelDepth)
        outside = true;
    ...
    [numthreads(X_BLOCK_SIZE, Y_BLOCK_SIZE, Z_BLOCK_SIZE)] void DownsampleMin(uint threadId
                                                                              : SV_GroupIndex,
                                                                                uint3 groupId
                                                                              : SV_GroupID,
                                                                                uint3 globalThreadId
...
[numthreads(X_BLOCK_SIZE, Y_BLOCK_SIZE, Z_BLOCK_SIZE)] void DownsampleAverage(uint threadId
                                                                              : SV_GroupIndex,
                                                                                uint3 groupId
                                                                              : SV_GroupID,
                                                                                uint3 globalThreadId
                                                                              : SV_DispatchThreadID)
{
    ...
    [numthreads(X_BLOCK_SIZE, Y_BLOCK_SIZE, Z_BLOCK_SIZE)] void DownsampleAverageInplace(uint threadId
                                                                                         : SV_GroupIndex,
                                                                                           uint3 groupId
                                                                                         : SV_GroupID,
                                                                                           uint3 globalThreadId
...
[numthreads(X_BLOCK_SIZE, Y_BLOCK_SIZE, Z_BLOCK_SIZE)] void Upsample(uint threadId
                                                                     : SV_GroupIndex,
                                                                       uint3 groupId
                                                                     : SV_GroupID,
                                                                       uint3 globalThreadId
...
[numthreads(X_BLOCK_SIZE, Y_BLOCK_SIZE, Z_BLOCK_SIZE)] void UpsampleBilinear(uint threadId
                                                                             : SV_GroupIndex,
                                                                               uint3 groupId
                                                                             : SV_GroupID,
                                                                               uint3 globalThreadId
                                                                             : SV_DispatchThreadID)
{
    ...
    // note: this is a slow version, can be made faster by using shared memory
    [numthreads(X_BLOCK_SIZE, Y_BLOCK_SIZE, Z_BLOCK_SIZE)] void UpsampleBilinearAccumulate(uint threadId
                                                                                           : SV_GroupIndex,
                                                                                             uint3 groupId
                                                                                           : SV_GroupID,
                                                                                             uint3 globalThreadId
...
[numthreads(X_BLOCK_SIZE, Y_BLOCK_SIZE, Z_BLOCK_SIZE)] void UpsampleBilinearAccumulateInplace(uint threadId
                                                                                              : SV_GroupIndex,
                                                                                                uint3 groupId
                                                                                              : SV_GroupID,
                                                                                                uint3 globalThreadId
                                                                                              : SV_DispatchThreadID)
{
    ...
    if (pos.x >= finerLevelWidth || pos.y >= finerLevelHeight || pos.z >= finerLevelDepth ||
        ...
        return; // position is out of bounds, we dont need to do anything
